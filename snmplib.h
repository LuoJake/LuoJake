...
//trap 类型
#define	coldStart	0	//代理进行了初始化
#define warmStart	1	//代理进行了重新初始化
#define linkDown	2	//一个接口从工作状态变为故障状态
#define linkUp		3	//一个接口从故障状态变为工作状态
#define authenticationFailure	4	//从SNMP管理进程接收到具有一个无效共同体的报文
#define egpNeighborLoss	5	//一个EGP相邻路由器变为故障状态
#define enterpriseSpecific	6	//代理自定义的事件，需要用后面的“特定代码”来指明

//SNMPV2c notice
#define No_Such_Object  0x02
#define No_Such_Instance  0x81
#define End_of_Mib_View  0x82
#define Genral_Error  0x05
#define Not_Writable  0x06
#define Inconsistent_Value  0x07

#define VALID_REQUEST(x)			((x == GET_REQUEST) || (x == GET_NEXT_REQUEST) || (x == GET_BULK_REQUEST) || (x == SET_REQUEST))
...
void insertRespLen(const int32 respStart, int32 size);
int32 parseVarBind(const int32 reqType, int32 index);
int32 parseSequence(const int32 reqType, int32 index);
int32 parseSequenceOf(const int32 reqType);
